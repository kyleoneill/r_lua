...
fn parse_block_pair(block_pair: Pair<Rule>) -> Result<lua_program::Block, CompileError> {
    ...
    Ok(lua_program::Block{ statements, return_statement })
}

fn parse_statement_pair(statement_pair: Pair<Rule>) -> Result<lua_program::Statement, CompileError> {
    ...
    match next.as_rule() {
        ...
        Rule::ListAssignment => {
            ...
            let var_list_pair = list_assign_inner.next().expect("ListAssignment must contain two inner values");
            let exp_list_pair = list_assign_inner.next().expect("ListAssignment must contain two inner values");
            let var_list = parse_var_list_pair(var_list_pair)?;
            let exp_list = parse_expression_list_pair(exp_list_pair)?;
            Ok(lua_program::Statement::MultipleAssignment(var_list, exp_list))
        },
        Rule::FunctionCall => {
            let mut function_call_inner = next.into_inner();
            let fc_name_pair = function_call_inner.next().expect("Rule::FunctionCall must have a first inner which is a name");
            let args_pair = function_call_inner.next().expect("Rule::FunctionCall must have a second inner which is an args");
            let args = parse_args_pair(args_pair)?;
            Ok(lua_program::Statement::FunctionCall(fc_name_pair.as_str().to_owned(), args))
        },
        ...
        Rule::DoBlockEndStatement => {
            ...
            let block_pair = do_block_inner.next().expect("Rule::DoBlockEndStatement must contain an inner block");
        ...
        Rule::WhileExprDoBlockStatement => {
            ...
            let expression_pair = while_expr_do_inner.next().expect("Rule::WhileExprDoBlockStatement must contain an inner expression");
            let block_pair = while_expr_do_inner.next().expect("Rule::WhileExprDoBlockStatement must contain an inner block");
            let expression = parse_expression_pair(expression_pair)?;
            let block = parse_block_pair(block_pair)?;
            Ok(lua_program::Statement::WhileExprDoBlockEnd(expression, block))
        },
        Rule::RepeatBlockUntilStatement => {
            let mut repeat_block_inner = next.into_inner();
            let block_pair = repeat_block_inner.next().expect("Rule::RepeatBlockUntilStatement must contain an inner block");
            let expression_pair = repeat_block_inner.next().expect("Rule::RepeatBlockUntilStatement must contain an inner expression");
            let block = parse_block_pair(block_pair)?;
            let expression = parse_expression_pair(expression_pair)?;
            Ok(lua_program::Statement::RepeatBlockUntilExpr(block, expression))
        },
        Rule::IfStatement => parse_if_statement_pair(next),
        Rule::ForEachStatement => {
            let mut for_each_inner = next.into_inner();
            let name_pair = for_each_inner.next().expect("Rule::ForEachStatement must have an inner name");
            let name = name_pair.as_str().to_owned();
            let expression_pair = for_each_inner.next().expect("Rule::ForEachStatement must have an inner expression");
            let first_expression = parse_expression_pair(expression_pair)?;
            let second_expression_pair = for_each_inner.next().expect("Rule::ForEachStatement must have a second inner expression");
            let second_expression = parse_expression_pair(second_expression_pair)?;
            let optional_expression = match for_each_inner.peek().is_some() && for_each_inner.peek().unwrap().as_rule() == Rule::Expression {
                true => {
            ...
            let block_pair = for_each_inner.next().expect("Rule::ForEachStatement must end with a block pair");
            let block = parse_block_pair(block_pair)?;
            Ok(lua_program::Statement::ForEach(name, first_expression, second_expression, optional_expression, block))
        },
        Rule::ForListStatement => {
            let mut for_list_inner = next.into_inner();
            let name_list_pair = for_list_inner.next().expect("Rule::ForListStatement must have a name list pair");
            let name_list = parse_name_list_pair(name_list_pair)?;
            let expr_list_pair = for_list_inner.next().expect("Rule::ForListStatement must have an expression list pair");
            let expr_list = parse_expression_list_pair(expr_list_pair)?;
            let block_pair = for_list_inner.next().expect("Rule::ForListStatement must have a block");
            let block = parse_block_pair(block_pair)?;
...
fn parse_if_statement_pair(if_statement_pair: Pair<Rule>) -> Result<lua_program::Statement, CompileError> {
    ...
    let expr_pair = if_inner.next().expect("Rule::IfStatement must have an expression pair");
    let expr = parse_expression_pair(expr_pair)?;
    let block_pair = if_inner.next().expect("Rule::IfStatement must have a block pair");
    let block = parse_block_pair(block_pair)?;
    ...
    while if_inner.peek().is_some() && if_inner.peek().unwrap().as_rule() == Rule::Expression {
        ...
        let elseif_block_pair = if_inner.next().expect("Rule::IfStatement must always have a block after an optional expression");
    ...
    Ok(lua_program::Statement::IfBlock((expr, block), elseif, else_block))
}

fn parse_return_statement_pair(statement_pair: Pair<Rule>) -> Result<lua_program::ReturnStatement, CompileError> {
    ...
    let expression_list = match inner.next() {
        Some(inner_val) => {
            match parse_expression_list_pair(inner_val) {
...
fn parse_function_name_pair(pair: Pair<Rule>) -> Result<lua_program::FunctionName, CompileError> {
    ...
    let outer_name_pair = inner.next().expect("Rule::FunctionName must have a name inner");
    ...
    let pass_self = match inner.next() {
        Some(final_pair) => Some(final_pair.as_str().to_owned()),
        None => None
    };
    Ok(lua_program::FunctionName { outer_name: outer_name_pair.as_str().to_owned(), accessors, pass_self })
}

fn parse_expression_list_pair(expr_list_pair: Pair<Rule>) -> Result<lua_program::ExpressionList, CompileError> {
    ...
    let mut inner = expr_list_pair.into_inner();
    while let Some(val) = inner.next() {
...
fn parse_var_list_pair(var_list_pair: Pair<Rule>) -> Result<lua_program::VarList, CompileError> {
    ...
    let mut inner = var_list_pair.into_inner();
    while let Some(val) = inner.next() {
...
fn parse_var_pair(var_pair: Pair<Rule>) -> Result<lua_program::Var, CompileError> {
    ...
    match next.as_rule() {
        Rule::VarNestedAccess => {
            ...
            let mut nested_access_inner = next.into_inner();
            while let Some(var_name) = nested_access_inner.next() {
...
fn parse_expression_pair(statement_pair: Pair<Rule>) -> Result<lua_program::Expression, CompileError> {
    ...
    match next.as_rule() {
        Rule::ExpressionInner => {
            ...
            Ok(lua_program::Expression::NormalExpression(expr_inner))
        },
        Rule::OperatorExpression => {
            let mut operator_exp_inner = next.into_inner();
            let first = operator_exp_inner.next().expect("OperatorExpression must have at least one inner");
            match first.as_rule() {
                Rule::ExpressionInner => {
                    let first_expr = parse_expression_inner_pair(first)?;
                    let operation = parse_binary_operator_pair(operator_exp_inner.next().unwrap());
                    let second_expr = parse_expression_inner_pair(operator_exp_inner.next().unwrap())?;
                    Ok(lua_program::Expression::BinaryExpr(operation, first_expr, second_expr))
                },
                Rule::UnaryOperator => {
                    let operation = parse_unary_operator_pair(first);
                    let expr = parse_expression_inner_pair(operator_exp_inner.next().unwrap())?;
                    Ok(lua_program::Expression::UnaryExpr(operation, expr))
                },
...
fn parse_expression_inner_pair(pair: Pair<Rule>) -> Result<lua_program::ExprInner, CompileError> {
    ...
    let mut inner = pair.into_inner();
    let first = inner.next().expect("Rule::ExpressionInner must contain an inner pair");
    let inner_str = first.as_str();
    if inner_str == "nil" { return Ok(lua_program::ExprInner::Nil) };
    if inner_str == "false" { return Ok(lua_program::ExprInner::Boolean(false)) };
    if inner_str == "true" { return Ok(lua_program::ExprInner::Boolean(true)) };
    match first.as_rule() {
        Rule::Numerical => {
            let number = parse_numerical_pair(first);
            Ok(lua_program::ExprInner::Numerical(number))
        },
        Rule::LiteralString => Ok(lua_program::ExprInner::LiteralString(first.as_str().to_owned())),
...
fn parse_binary_operator_pair(pair: Pair<Rule>) -> lua_program::BinaryOperator {
    ...
    match inner.as_str() {
        ...
        "<=" => lua_program::BinaryOperator::BooleanOperator(lua_program::BooleanOperator::LessThanEqualTo),
        ">" => lua_program::BinaryOperator::BooleanOperator(lua_program::BooleanOperator::GreaterThan),
        ">=" => lua_program::BinaryOperator::BooleanOperator(lua_program::BooleanOperator::GreaterThanEqualTo),
...
fn parse_numerical_pair(pair: Pair<Rule>) -> lua_program::NumberKind {
    ...
    let mut inner = pair.into_inner();
    let next = inner.next().expect("Rule::Numerical must contain an inner value");
    match next.as_rule() {
        Rule::Float => {
            match next.as_str().parse::<f64>() {
                Ok(float) => lua_program::NumberKind::Float(float),
                Err(_) => panic!("Failed to parse a Rule::Float into an f64")
            }
        },
        Rule::Integer => {
            match next.as_str().parse::<i64>() {
...
fn parse_params_pair(pair: Pair<Rule>) -> Result<lua_program::Parameters, CompileError> {
    ...
    let mut inner = pair.into_inner();
    let next = inner.next().expect("Rule::ParList must have at least one inner value");
    match next.as_rule() {
        Rule::NameList => {
            let name_list = parse_name_list_pair(next)?;
            let expansion = match inner.next() {
                Some(_) => Some(lua_program::Expansion),
                None => None
            };
